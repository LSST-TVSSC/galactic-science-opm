...
Note that this will not automatically run the e2e tests without overriding the `BASE_URL`, 
because it wants to run against the frontend-proxy as defined in `compose.e2e.yaml` but it
does not exist here. 
If you want to run the tests immediately, use this:
```sh
...
Can also be run in your local directory, because nothing django related should be used in these tests:
`python manage.py test custom_code.tests.unit --settings=galactic_science_opm.settings_test` or
`pytest custom_code/tests/unit`
...
or 
`pytest custom_code/tests/integration`
...
The start up will also run the management command `seed_e2e_data`, which clears the 
test-database and adds testing data. For this to work, you have to put the file 
`db_out_full.json` in the folder specified in the env variable `DJANGO_TEST_DATA_DIR`.
As this file might at some point contain data that is proprietary, this file is not committed
for now. This might change in the future. To get this file, reach out to another dev.
...
When you run the E2E tests again, though, the database is reset and all the 
