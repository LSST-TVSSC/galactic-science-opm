...
def test_unauthorized_user_can_not_view_page(page: Page):
    ...
    expect(target_rows).to_have_count(NUMBER_OF_ELEMENTS_PER_PAGE_UNAUTH)
    expect(targets_table).to_contain_text(re.compile(rf".*{LOGIN_REQUEST_TEXT}"))
...
def test_authorized_user_can_view_page(page: Page):
    ...
    expect(targets_pagination).to_contain_text(
        re.compile(rf"of {NUMBER_OF_ELEMENTS_TOTAL}")
    )

    for row in TEST_TARGETS:
        name, target_type, obs, saved, pk = (
            row["name"],
            row["type"],
            row["obs"],
            row["saved"],
        ...
        expect(targets_table).to_contain_text(
            re.compile(rf"{name}\s+{target_type}\s+{obs}\s+{saved}\s+")
        )
...
def test_authorized_user_can_create_target(page: Page):
    ...
    target_page.open_it()
    target_page.login(*VALID_USER_CREDENTIALS)
    target_page.create_target(
        name=NON_EXISTENT_TARGET, ra=NON_EXISTENT_RA, dec=NON_EXISTENT_DEC
    )

    expect(page).to_have_title(re.compile(rf".*{NON_EXISTENT_TARGET}"))
...
def test_authorized_user_can_merge_targets(page: Page):
    ...
    expect(page).to_have_title(re.compile(rf".*{TO_MERGE[0]}"))

    target_info = page.get_by_test_id("target-info")
    for m in TO_MERGE:
        expect(target_info).to_contain_text(re.compile(rf".*{m}"))
