...
def test_shows_top_targets_in_descending_order(page: Page):
    ...
    for _i, target_data in enumerate(TOP_TARGETS):
        target_name, coordinates, pk = (
            target_data["name"],
            target_data["coordinates"],
            target_data["pk"],
        )
        target = featured_targets.filter(has_text=re.compile(rf".*{target_name}.*"))
        ...
        expect(image_wrapper).to_have_attribute("href", f"/targets/{pk}/")
        ...
        for link_data in BROKER_LINKS:
            ...
            expect(link).to_have_attribute("href", href.format(target_name))

        # all targets link
        all_targets_link = page.get_by_role(
            "link", name="View ranked microlensing targets (2026)"
        )
        ...
        expect(footer_impressum_link).to_have_attribute(
            "href", "https://www.uni-heidelberg.de/de/impressum"
        )

        footer_imprint_link = page.get_by_role("link", name="Imprint")
        expect(footer_imprint_link).to_be_visible()
        expect(footer_imprint_link).to_have_attribute(
        ...
        expect(footer_datenschutz).to_have_attribute(
            "href", "https://www.uni-heidelberg.de/de/datenschutzerklaerung"
        )

        footer_privacy = page.get_by_role("link", name="Privacy Statement")
        expect(footer_privacy).to_be_visible()
        expect(footer_privacy).to_have_attribute(
...
def test_git_hash_is_visible(page: Page):
    ...
    expect(commit_hash_container).to_contain_text(
        re.compile(f"{os.environ.get('GIT_COMMIT')}")
    )
