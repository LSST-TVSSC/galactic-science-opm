...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('events', help='Number of events to be ingested, = number of pages')
    ...
    def handle(self, *args, **options):
        ...
        (list_of_targets, new_targets) = Alerce.fetch_alerts(events=int(str(options['events'])),
                                                             days=int(str(options['days'])))

        print('Identified and ingested '+str(len(list_of_targets))+' target(s) from ALeRCE')
        if full_phot:
