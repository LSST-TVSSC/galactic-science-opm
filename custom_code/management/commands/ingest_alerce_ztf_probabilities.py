...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help="filter for targets containing ... (e.g. ZTF26)",
        )
    ...
    def handle(self, *args, **options):
        ...
        new_or_modified_targets = (
            ...
            .filter(name__icontains=str(options["target_name_contains"]))
            .distinct()
        )
        ...
        for target in new_or_modified_targets:
            print(
                f"Check lc_classifier_BHRF_forced_phot microlensing probability for event {target.name}"
            )
            alerce = Alerce()
            try:
                if "ZTF" in "".join([x for x in target.names if "ZTF" in x]):
                    ...
                    try:
                        ...
                        if "feature_antares_devkit_version" in antares_locus.properties:
                            antares_version = antares_locus.properties[
                                "feature_antares_devkit_version"
                            ]
                        else:
                            antares_version = "0.0"
                        antares_probability = 0.0
                        chi2_red_keys = [
                            antares_locus.properties[x]
                            for x in antares_locus.properties
                            if "chi2" in x and "microlensing" in x
                        ]
                    ...
                    try:
                        antares_locus = get_by_lsst_dia_object_id(
                            str(target_name_lsst[5:])
                        )
                        if "feature_antares_devkit_version" in antares_locus.properties:
                            antares_version = antares_locus.properties[
                                "feature_antares_devkit_version"
                ...
                if "ZTF" in "".join([x for x in target.names if "ZTF" in x]):
                    best_class = max(
                        [
                            ...
                            if "lc_classifier_BHRF_forced_phot"
                            == item["classifier_name"]
                        ]
                    )[1]
                else:
                    best_class = max(
                        [
                            ...
                            for item in probabilities
                            if "stamp_classifier_rubin_beta" == item["classifier_name"]
                        ]
                    )[1]
                print(best_class)
                with transaction.atomic():
                    GalacticTarget.objects.filter(name=target).update(
                        target_type=f"{best_class} candidate"
                    )
            except Exception as e:
                print(f"Exception: {e}")
                best_class = ""
            ...
            if "Microlensing" in best_class:
                ...
                prob_class2 = 0.0
                try:
                    stochastic_bhrf_prob = prob_pd.loc[
                        prob_pd["classifier_name"] == "lc_classifier_BHRF_forced_phot"
                    ]
                    prob_class1 = float(
                        stochastic_bhrf_prob[
                            stochastic_bhrf_prob["class_name"] == "Microlensing"
                        ]["probability"].iloc[0]
                    )
                    prob_class2 = float(
                        stochastic_bhrf_prob[
                            stochastic_bhrf_prob["class_name"] == "CV/Nova"
                        ]["probability"].iloc[0]
                    )
                except Exception as e:
                ...
                prob_class3 = 0.0
                try:
                    bogus_prob = prob_pd.loc[
                        prob_pd["classifier_name"] == "stamp_classifier"
                    ]
                    prob_class3 = float(
                        bogus_prob[bogus_prob["class_name"] == "bogus"][
                            "probability"
                        ].iloc[0]
                    )
                except Exception as e:
                ...
                prob_class4 = 0.0
                try:
                    forced_atat_prob = prob_pd.loc[
                        prob_pd["classifier_name"]
                        == "LC_classifier_ATAT_forced_phot(beta)"
                    ]
                    prob_class4 = float(
                        forced_atat_prob[
                            forced_atat_prob["class_name"] == "Microlensing"
                        ]["probability"].iloc[0]
                    )
                except Exception as e:
                    print(f"Classification missing, Exception: {e}")
