...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "events", help="name of a specific event, all or an integer number"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        (list_of_targets, new_targets) = Ogle.fetch_alerts(
            years=year_list, events=str(options["events"])
        )

        print(
            "Identified and ingested "
            + str(len(list_of_targets))
            + " target(s) from OGLE survey"
        )
        ...
        if str(options["events"]).isnumeric():
            selected_targets = Ogle.select_random_targets(
                list_of_targets, new_targets, ntargets=int(options["events"])
            )
            print(
                "Ingesting data from "
                + str(len(selected_targets))
