        ...
        with tempfile.TemporaryDirectory():
            ...
            with transaction.atomic():
                ...
                target_coord = SkyCoord(
                    ra=target.ra, dec=target.dec, unit="deg", frame="icrs"
                )
                ...
                if "ZTF" in str(target.name):
                    ...
                    location = EarthLocation.from_geodetic(
                        lat="-30d14m40.68s", lon="-70d44m57.90s", height=2647.0 * u.m
                    )
                for reduced_datum in photometry:
                    t = Time(reduced_datum.timestamp, scale="utc")
                    ltt_heliocentric = t.light_travel_time(
                        target_coord, kind="heliocentric", location=location
                    )
                    ...
                    try:
                        ...
                        if age < TimeDelta(10 * 365.0, format="jd"):
                ...
                if "filter" in df.columns:
                    ...
                    for category in unique_categories:
                        ...
                        if len(filtered_df) > 2 and len(str(category)) < 100:
                            ...
                            try:
                                filtered_df.to_csv(
                                    path.join(data_dir, f"{category}.dat"),
                                    columns=["magnitude", "error", "timestamp"],
                ...
                if len(data) > 3:
                    ...
                    try:
                        ...
                        model_path = path.join(
                            tempdirname, "FinalModels", model_name[0]
                        )
                        model_results = ModelResults(model_path)
                        if (
                            model_results.model_parameters.u0_error
                            + model_results.model_parameters.u0
                            < 5.0
                            and model_results.model_parameters.tE > 0.0
                            and model_results.model_parameters.u0_error > 0.0
                        ) or "LSST" in target.name:
    ...
    def handle(self, *args, **options):
        if str(options["event"]) != "ZTF" and str(options["event"]) != "LSST":
            ...
            new_or_modified_targets = (
                ...
                .filter(name__icontains=str(options["event"]))
            ...
            distinct_ids = (
                ...
                .distinct("target_id")
                .filter(target__name__icontains=str(options["event"]))
            ...
            if str(options["event"]) == "LSST":
                qs = MicrolensingRadarData.objects.filter(
                    target__name__icontains=str(options["event"])
                ).distinct()
            else:
                qs = (
            ...
            target_names = list(
                set(
                    [x.target.name for x in qs]
                    + [x.name for x in new_or_modified_targets]
                )
            )
            for target_name in target_names:
                target = GalacticTarget.objects.filter(
                    name__icontains=target_name
                ).last()
                # 2d buffer: to ensure broker match against ZTF and/or baseline phot has sufficient time
                if not target.ztf_baseline_checked and target.created < (
                    timezone.now() - timedelta(days=2)
                ):
                    baseline_photometry_r = query_ztf_lightcurve(
                        target.ra, target.dec, 2.0, start_mjd=58500.0, passband="r"
                    )
                    baseline_photometry_g = query_ztf_lightcurve(
                        target.ra, target.dec, 2.0, start_mjd=58500.0, passband="g"
                    )
                    baseline_photometry_i = query_ztf_lightcurve(
                        target.ra, target.dec, 2.0, start_mjd=58500.0, passband="i"
                    )
                    baseline_photometry = pd.concat(
                        [
                            baseline_photometry_r,
                            baseline_photometry_g,
                            baseline_photometry_i,
                        ],
                        ignore_index=True,
                    )
                    ...
                    if (
                        ...
                        and "mjd" in baseline_photometry.columns
                    ):
