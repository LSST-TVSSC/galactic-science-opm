    ...
    def handle(self, *args, **options):
        ...
        for target in target_list:
            ...
            if target_classification.exists():
                try:
                    psip = psi_planet_priority_peak(
                        microlensing_model.u0,
                        microlensing_model.err_u0,
                        microlensing_model.tE,
                        microlensing_model.err_tE,
                        sigma_threshold=0.5,
                    )
                ...
                if target_classification.filter(source="fink_ZTF").exists():
                    try:
                        prob_fink = (
                            target_classification.filter(source="fink_ZTF")
                            .latest()
                            .prob_class1
                        )
