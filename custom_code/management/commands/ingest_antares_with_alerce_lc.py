...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--all_years",
            help="Considers all ZTF years in ingest [optional]: True or False",
        )
    ...
    def handle(self, *args, **options):
        ...
        (list_of_targets, new_targets) = Antares.fetch_alerts(
            days=int(str(options["days"])), all_years=all_years
        )
        print(
            "Identified "
            + str(len(list_of_targets))
            + " target(s) from ANTARES microlensing filter"
        )
        if full_phot:
