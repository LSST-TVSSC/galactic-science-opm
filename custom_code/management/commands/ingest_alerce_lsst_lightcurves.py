...
from custom_code.brokers import alerce_lsst
import numpy as np
...
class Command(BaseCommand):
    help = (
        "Populate the database with lightcurves of ALeRCE ZTF microlensing candidates"
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "n_event_pages", help="Number of events to be ingested, = number of pages"
        )
        parser.add_argument("days", help="days firstmjd before now")
        parser.add_argument(
            "--phot", help="Force ingest of full photometry [optional]: True or False"
        )

    def handle(self, *args, **options):
        print("Starting ALeRCE photometry ingest")
        Alerce = alerce_lsst.ALERCEBroker()
        full_phot = False
        ...
        (list_of_targets, new_targets) = Alerce.fetch_alerts(
            events=int(str(options["n_event_pages"])), days=int(str(options["days"]))
        )
