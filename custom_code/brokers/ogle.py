...
class OGLEBroker(GenericBroker):
    ...
    def fetch_lens_model_parameters(self, years):
        ...
        print(
            "OGLE harvester: Fetching event model parameters for years " + repr(years)
        )
        ...
        for year in years:
            ...
            print(
                "OGLE harvester: retrieving parameters for events from "
                + str(year)
                + " with status "
    ...
    def ingest_events(self, ogle_events, debug=False):
        ...
        for event_name, event_params in ogle_events.items():
            ...
            if len(qs) == 0:
                ...
                target, result = validators.get_or_create_event(
                    event_name, s.ra.deg, s.dec.deg, debug=debug
                )
        ...
        print(
            "OGLE harvester: completed ingest of events, including "
            + str(len(new_targets))
            + " new targets"
        )
            ...
            try:
                ...
                if int(year) > 1990:
                    try:
                        ...
                        print(
                            "OGLE harvester: WARNING malformed photometry for event "
                            + target.name
                            + ", skipping ingest"
                        )
                ...
                print(
                    "OGLE harvester: Encountered malformed target name "
                    + target.name
                    + ", skipped ingest"
                )
    ...
    def read_ogle_lightcurve(self, target):
        ...
        if "OGLE" in ogle_name:
            ...
            print(
                "OGLE harvester: read and ingested photometry for event "
                + target.name
                + " from file "
                + os.path.join(event.lower(), "phot.dat")
            )

        else:
            print(
                "OGLE Harvester WARNING: No OGLE name available for target "
                + target.name
                + " so cannot find lightcurve to ingest"
            )
        ...
        for i in range(0, len(photometry), 1):
            ...
            try:
                ...
                if "unique_photometry" in str(e):
                    ...
                    print(
                        "OGLE HARVESTER: Encountered exception during photometry ingest for target "
                        + target.name
                    )
                    print(e)
