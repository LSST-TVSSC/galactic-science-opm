...
def make_target_tags_list(target):
    ...
    params = (
        MicrolensingParameterModel.objects.filter(target=target_radar)
        .order_by("-updated_at")
        .only("t0", "tE")
        .first()
    )
    ...
    if t0 is not None and tE is not None:
        try:
            ...
            if (
                float(t0) - float(tE) < current_jd < float(t0) + float(tE)
                and err_tE < tE
            ):
                tags.append(
                    {
                        "class": "active",
                        "text": "Microlensing event current t in [t0-tE,t0+tE]",
                    }
                )
